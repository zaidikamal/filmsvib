...
export default async function MyArticlesPage() {
  ...
  const STATUS_CONFIG: Record<string, any> = {
    ...
    rejected: { label: "مرفوض", color: "bg-[#d4af37]/10 text-[#d4af37] border-[#d4af37]/20", icon: "❌" },
        ...
        {!articles || articles.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {articles.map((article) => {
              ...
              return (
                <div key={article.id} className="group bg-[#12121a]/50 backdrop-blur-md rounded-[2.5rem] overflow-hidden border border-white/5 hover:border-[#d4af37]/30 transition-all flex flex-col h-full shadow-2xl relative">
                  ...
                  <div className="p-8 flex-1 flex flex-col">
                    ...
                    {article.rejection_reason && status === 'rejected' && (
                      <div className="mb-6 p-4 bg-[#d4af37]/5 border border-[#d4af37]/10 rounded-2xl">
                        <p className="text-[10px] font-black text-[#d4af37] uppercase tracking-widest mb-1">سبب الرفض:</p>
                    ...
                    <div className="mt-auto pt-6 border-t border-white/5 flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        ...
                        {(status === 'draft' || status === 'rejected') && (
                          <Link href={`/news/edit/${article.id}`} className="p-3 bg-[#d4af37]/10 hover:bg-[#d4af37]/20 text-[#d4af37] rounded-xl transition-all border border-[#d4af37]/20">
                            ✏️ تعديل
                          </Link>
                        )}
                      </div>
